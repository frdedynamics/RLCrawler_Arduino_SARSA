...
int newgwS1 = 0;
int newgwS2 = 0;
float Q_State[4] = {0, 0, 0, 0};
int movedir;
float del;

int Q_init = 0;
float e_greed = 90;         //latest val: 50; 93; // factor for  e-greedy action selection in range from 0-99; balance between exploration & exploitation
...
//float alpha_step = 0.01; //0.0005 works
float gamma = 0.95; //latest val 0.8// 0.5 at 9states worked 0.7 //discount factor of Q-value of next state
...
void setup()
{
  ...
  for (i = 0; i < 4; i++)
  {
    Q_State[i] = Q[gw2state[gwS1][gwS2]][i];
  }
  action = e_greedy(e_greed, e_greed_final, e_greed_step, Q_State);
  delay(2000);
  Serial1.print("g1: ");
  Serial1.print(gwS1);
  Serial1.print("g2: ");
...
void loop()
{
  ...
  DEBUG_PRINTLN("****************");
  DEBUG_PRINT("OLD STATE: ");
  DEBUG_PRINTLN(gw2state[gwS1][gwS2]);
  ...
  DEBUG_PRINTLN(gw2state[newgwS1][newgwS2]);
  ...
  for (i = 0; i < 4; i++)
  {
    Q_State[i] = Q[gw2state[newgwS1][newgwS2]][i];
  }
  newaction = e_greedy(e_greed, e_greed_final, e_greed_step, Q_State);
  ...
  //compute Q update
  //SARSA
  q_sarsa = Q[gw2state[newgwS1][newgwS2]][newaction];
  delta_q = reward + (gamma * q_sarsa) - Q[gw2state[gwS1][gwS2]][action];
